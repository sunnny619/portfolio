---
title: SSU MATE 서비스 개발일지 02 - PDF 파서 만들기
tag: 개발
date: 2026-09-12
summary: 요구사항 정리 이후 동아리 활동보고서와 회의록 PDF를 구조화된 데이터로 바꾸기 위해 pdfplumber를 적용한 기록
---
## 들어가며

저는 대학 행정 AI 에이전트 '슈메이트'를 만들고 있습니다. 교내에 흩어져 있는 데이터를 모아 실제 업무와 연결하는 서비스입니다.

지난 글에서는 창업지원단 담당자 인터뷰를 바탕으로 문제를 정의하고, 요구사항과 초기 기능 명세를 정리했습니다. 그때 정한 핵심 흐름은 “질문 → 자료 조회 → 원문 확인 → 근거가 포함된 기획안 초안 작성”이었습니다.

이 흐름을 만들려면 먼저 원문 자료를 데이터로 바꿔야 했습니다.
...
이제 PDF에서 값을 꺼내는 방법은 어느 정도 확인했습니다. 다음 단계는 이렇게 추출한 데이터를 어디에, 어떤 구조로 저장할지 정하는 일이었습니다. 그래서 다음 글에서는 기능명세서와 ERD를 바탕으로 AWS RDS에 PostgreSQL 데이터베이스를 구축한 과정을 정리해보려고 합니다.
